...
class P4Headers(object):
    ...
    def to_string(self):
        return '\n'.join("%s" % h.to_string() for name, h in self.headers.items()) \
                ...
                + '\n' + \
                '\n'.join("%s" % n.to_string() for name, n in self.struct.items())
    def __str__(self):
    ...
    def register_struct(self, struct):
        if not isinstance(struct, P4Header):
            raise TypeError("P4Header")
        else:
            self.struct[struct.instance_name] = struct
    ...
    def declare_ipv4(self):
        ...
        ipv4_diffserv = P4HeaderField("diffserv", 0, 8)
        ...
        ipv4_checksum = P4HeaderField("hdrChecksum", 0, 16)
        ipv4_src = P4HeaderField("srcAddr", 0, 32)
        ipv4_dst = P4HeaderField("dstAddr", 0, 32)
    ...
    def declare_metadata(self):
        metadata = P4Header("struct","metadata",1)
        lsh_select = P4HeaderField("lsh_select", 0, 4)
        flag = P4HeaderField("flag", 0, 1)
        key = P4HeaderField("key", 0, 32)

        metadata.add_field(lsh_select)
        metadata.add_field(flag)
        metadata.add_field(key)
        self.register_struct(metadata)
    ...
    def declare_header(self):
        myheader = P4Header("struct", "header_t", 0)
        eth = P4HeaderField("ethernet_t",-1,0)
        ipv4 = P4HeaderField("ipv4_t",-1,0)
        tcp = P4HeaderField("tcp_t",-1,0)
        mafia = P4HeaderField("mafia_metadata_t",-1,0)
        
        myheader.add_field(eth)
        myheader.add_field(ipv4)
        myheader.add_field(tcp)
        myheader.add_field(mafia)

        self.register_struct(myheader)


class P4Header(object):
    def __init__(self, type_name, instance_name, is_metadata,is_header = 0, requires_format = 1):
    ...
    def to_string(self):
        if(self.type_name=="struct"):
            self_str = p4header_struct % (self.instance_name, indent_str(self.fields.to_string(), 2))
        else:
...
class P4HeaderField(object):
    ...
    def to_string(self):
        if(self.value!=-1):
            return "bit<%d>\t%s;" % (self.width, self.name)
        else:
            return "%s %s;" %(self.name,self.name[0:-2])
    def __str__(self):
        return self.to_string()
